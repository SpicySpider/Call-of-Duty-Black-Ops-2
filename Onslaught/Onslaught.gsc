...
init()
{
	...
	level.cmPerkJuggHealth = getDvarIntDefault( "cmPerkJuggHealth", 500 );
	...
	level.cmPlayerMaxHealth = getDvarIntDefault( "cmPlayerMaxHealth", 181 );
	level.player_starting_points = getDvarIntDefault( "cmPlayerStartingPoints", 30000 );
	//phd min explosion damage
	level.cmPerkMinPhdExplosionDamage = getDvarIntDefault( "cmPerkMinPhdExplosionDamage", 10000 );
	level.zombie_vars[ "zombie_perk_divetonuke_min_damage" ] = level.cmPerkMinPhdExplosionDamage;
	//phd max explosion damage
	level.cmPerkMaxPhdExplosionDamage = getDvarIntDefault( "cmPerkMaxPhdExplosionDamage", 62500 );
	level.zombie_vars[ "zombie_perk_divetonuke_max_damage" ] = level.cmPerkMaxPhdExplosionDamage;
	//phd explosion radius
	level.cmPerkPhdDamageRadius = getDvarIntDefault( "cmPerkPhdDamageRadius", 1000 );
	level.zombie_vars[ "zombie_perk_divetonuke_radius" ] = level.cmPerkPhdDamageRadius;
	//points lost if down
	level.cmPlayerDownedPointsLostSelf = getDvarFloatDefault( "cmPlayerDownedPointsLostSelf", 1.00 );
	level.zombie_vars[ "penalty_downed" ] = level.cmPlayerDownedPointsLostSelf;
	//spawn rate
	level.cmZombieSpawnRate = getDvarFloatDefault( "cmZombieSpawnRate", 0.08 );
	level.zombie_vars[ "zombie_spawn_delay" ] = level.cmZombieSpawnRate;
	//powerup drop rate
	level.cmPowerupDropRate = getDvarIntDefault( "cmPowerupDropRate", 10000 );
	level.zombie_vars["zombie_powerup_drop_increment"] = level.cmPowerupDropRate;
	
	//zombie speed multiplier
	level.cmZombieMoveSpeedMultiplier = getDvarIntDefault( "cmZombieMoveSpeedMultiplier", 10000 );
	...
	level.cmPowerupMaxPerRound = getDvarIntDefault( "cmPowerupMaxPerRound", 40 );
...
onplayerspawned()
{
	...
	for(;;)
	{
		...
		if (self.initial_spawn)
		{
			...
			self iprintlnbold("Welcome^2 "+self.name +"^7 to ^3SpicySpider's^1 ONSLAUGHT");
			wait 6;
			self iprintlnbold("^2" +self.name +"^7, GET READY TO GET ^1FUCKED");
			wait 1;
			EASTEREGGSONG(played);
			wait 4;
			self iprintlnbold("And ^2" +self.name +"^7 remember, DON'T ^1DIE");
			wait 4;
				
			self takeweapon("c96_zm");
			self giveweapon("raygun_mark2_upgraded_zm", 0, self get_pack_a_punch_weapon_options("raygun_mark2_upgraded_zm"));
		...
		// Do things specific for SpicySpider.
		//if ( self.name == "SpicySpider" ){
		//self iprintln("^1                                                                                                                     WELCOME MR. SPICYSPIDER, ENJOY YOUR MODS");
		//self giveweapon("staff_air_upgraded_zm");
		//self givemaxammo("staff_air_upgraded3_zm");
		//self givemaxammo("staff_air_upgraded_zm");
		//self givemaxammo("staff_air_upgraded2_zm");
		...
		//if ( self.name == "Joey Winchester" ){
		//self iprintln("^1                                                                                                                     WELCOME MR. Joey Winchester, ENJOY YOUR MODS");
...
Spawnbitches(amountofbitches)
{
	for(i=0;i<amountofbitches;i++)
	{
...
post_all_players_spawned()
{
	...
	level.zombie_move_speed = 100000;
	//level.zombie_vars[ "zombie_spawn_delay" ] = 0.10;
...
set_movement_dvars()
{
	...
	setdvar( "player_sprintStrafeSpeedScale", 1 );
						//setdvar( "player_enduranceSpeedScale", 3 );
						//setdvar( "player_sprintSpeedScale", 5 );
	
	setdvar( "g_speed", 190 );

	setdvar( "player_reviveTriggerRadius", 1000 );
...
zombie_health_fix()
{
	for ( ;; )
	{
		...
		if(level.zombie_health > 500000)
		{
			level.zombie_health = 500000;
		}
	}
}
